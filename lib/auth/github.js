...
module.exports = function() {
	return githubAuth()
		.then(token => {
		  if (token) {
		    console.log(chalk.green('Sucessfully authenticated to a Virtual Tour repository!'));
		  }
		})
		.catch(err => {
	    switch (err.code) {
	      case 401:
	        console.log(chalk.red('Couldn\'t authenticate to a repository. Please try again.'));
	        break;
	      case 403:
	        console.log(chalk.red('Couldn\'t authenticate to this repository with the stored access token.'));
	        break;
	      case 422:
