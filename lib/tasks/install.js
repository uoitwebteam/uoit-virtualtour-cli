    ...
    return new Listr([{
      ...
      task: () => {
      	...
      	return new Listr([{
      		...
      		task: (c, task) => execa('npm', ['install'], { cwd: PREFERENCES.serverPath })
				  	.then(() => {
      ...
      task: () => {
      	...
      	return new Listr([{
      		...
      		title: 'Installing dependencies...',
      		task: (c, task) => execa('npm', ['install'], { cwd: PREFERENCES.clientPath })
				  	.then(() => {
				  		task.title = `${ chalk.dim.white(task.title) } Dependencies installed!`
				  	})
      	}])
      ...
      task: () => {
      	...
      	return new Listr([{
      		...
      		title: 'Installing dependencies...',
      		task: (c, task) => execa('npm', ['install'], { cwd: PREFERENCES.mapPath })
				  	.then(() => {
				  		task.title = `${ chalk.dim.white(task.title) } Dependencies installed!`
				  	})
      	}])
